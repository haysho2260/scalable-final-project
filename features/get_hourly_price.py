...
def process_hourly_prices():
    ...
    for f in source_files:
        ...
        if not os.path.exists(out_path):
            ...
            try:
                ...
                date_col_src = next((c for c in src_df.columns if 'date' in c.lower()), 'Date')
                date_col_tgt = next((c for c in tgt_df.columns if 'date' in c.lower()), 'Date')
                
                if date_col_src in src_df.columns and date_col_tgt in tgt_df.columns and 'HE' in src_df.columns and 'HE' in tgt_df.columns:
                     src_df[date_col_src] = pd.to_datetime(src_df[date_col_src], errors='coerce')
                     tgt_df[date_col_tgt] = pd.to_datetime(tgt_df[date_col_tgt], errors='coerce')
                     ...
                     if not src_df.empty and not tgt_df.empty:
                         ...
                         src_max = src_df.sort_values([date_col_src, 'HE']).iloc[-1]
                         tgt_max = tgt_df.sort_values([date_col_tgt, 'HE']).iloc[-1]
                         ...
                         if src_ts > tgt_ts:
                             needs_update = True
                             print(f"Update needed for {f}: Source latest {src_ts} > Target latest {tgt_ts}")
    ...
    if not files_to_process:
        ...
        print(f"Found {len(files_to_process)} files needing update. Fetching prices...")
        ...
        for f in files_to_process:
            ...
            try:
                ...
                price_row = prices_df[(prices_df['Year'] == year) & (prices_df['Month'] == month)]
                ...
                if not price_row.empty:
                    ...
                    if 'CAISO Total' in df.columns:
                        df['Estimated_Hourly_Cost_USD'] = df['CAISO Total'] * 1000 * (price / 100.0)
                else:
...
def process_lag_prices():
    ...
    if not os.path.exists(source_dir):
        print("Source directory hourly_price does not exist. Run process_hourly_prices first.")
        return

    source_files = [f for f in os.listdir(source_dir) if f.startswith("CAISO_Price_") and f.endswith(".csv")]
    ...
    for f in source_files:
        try:
            parts = f.replace("CAISO_Price_", "").replace(".csv", "").split("_")
            year, month = int(parts[0]), int(parts[1])
    ...
    for meta in file_metadata:
        ...
        current_idx = next(i for i, m in enumerate(file_metadata) if m['file'] == f)
        start_idx = max(0, current_idx - 2)
        context_files = file_metadata[start_idx : current_idx + 1] # Include current
        ...
        if target_col not in full_df.columns: continue
        
        # Compute Daily Stats
        daily_stats = full_df.groupby('Daily_Date')[target_col].agg(['mean', 'std']).reset_index()
        daily_stats.columns = ['Daily_Date', 'daily_mean_cost', 'daily_std_cost']
        daily_stats['Daily_Date'] = pd.to_datetime(daily_stats['Daily_Date'])
        
        # Lags
        lags = [1, 7, 15, 30]
        for lag in lags:
            daily_stats[f'daily_mean_cost_lag_{lag}'] = daily_stats['daily_mean_cost'].shift(lag)
            daily_stats[f'daily_std_cost_lag_{lag}'] = daily_stats['daily_std_cost'].shift(lag)
            
        full_df['Daily_Date'] = pd.to_datetime(full_df['Daily_Date'])
        
        # Clean existing lags
        cols_to_drop = [c for c in full_df.columns if 'daily_mean_cost' in c or 'daily_std_cost' in c]
        if cols_to_drop: full_df = full_df.drop(columns=cols_to_drop)
        ...
        final_df = full_df[(full_df['TempYear'] == target_year) & (full_df['TempMonth'] == target_month)].copy()
